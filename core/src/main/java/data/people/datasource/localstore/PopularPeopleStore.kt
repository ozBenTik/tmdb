...
class PopularPeopleStore @Inject constructor() {
    ...
    fun insert(page: Int, people: List<PopularActor>) {
        if (page == 1) {
            deleteAll()
    ...
    fun observeEntries(): SharedFlow<Map<Int, List<PopularActor>>> = _people.asSharedFlow()

    private fun updatePage(page: Int, people: List<PopularActor>) {
