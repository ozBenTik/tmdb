...
class TopRatedMoviesCarrouselAdapter(
    private val tmdbImageUrlProvider: TmdbImageUrlProvider,
    private val onItemClickListener: (movieId: Int) -> Unit,
) :
    ListAdapter<Movie, TopRatedMovieCarrouselViewHolder>(TopRatedMovieDiff) {

    override fun onCreateViewHolder(
        parent: ViewGroup,
        viewType: Int
    ): TopRatedMovieCarrouselViewHolder {
        val binding = MovieCardBinding.inflate(
            LayoutInflater.from(parent.context), parent, false
        )
        return TopRatedMovieCarrouselViewHolder(binding)
    }

    override fun onBindViewHolder(holder: TopRatedMovieCarrouselViewHolder, position: Int) {
...
class TopRatedMovieCarrouselViewHolder(
    internal val binding: MovieCardBinding
) : RecyclerView.ViewHolder(binding.root)

object TopRatedMovieDiff : DiffUtil.ItemCallback<Movie>() {
