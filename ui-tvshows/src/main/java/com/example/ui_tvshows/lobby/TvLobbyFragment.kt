            ...
            viewModel.state.collect { uiState ->
                (view as ComposeView).setContent {
                    SwipeRefresh(
                        ...
                        TvShowsLobbyScreen(
                            uiState = uiState,
                            provideFitImageUrl = { image: String -> tmdbImageUrlProvider.getBackdropUrl(image, 800)},
                            onTvShowSelected = { tvId ->
                                Toast.makeText(requireContext(), "TVSHOW $tvId selected", Toast.LENGTH_SHORT).show()
                            }
                        ) {
                            viewModel.logout()
                        }
